...
#include "../include/olm/outbound_group_session.h"

#include <string.h>
//#include <time.h>

#include "../include/olm/base64.h"
#include "../include/olm/cipher.h"
#include "../include/olm/crypto.h"
#include "../include/olm/error.h"
#include "../include/olm/megolm.h"
#include "../include/olm/memory.h"
#include "../include/olm/message.h"
#include "../include/olm/pickle.h"
#include "../include/olm/pickle_encoding.h"
...
__declspec(dllexport) size_t olm_outbound_group_session_size() {
    return sizeof(OlmOutboundGroupSession);
}

__declspec(dllexport) OlmOutboundGroupSession * olm_outbound_group_session(
    void *memory
) {
...
__declspec(dllexport) const char *olm_outbound_group_session_last_error(
...
__declspec(dllexport) size_t olm_pickle_outbound_group_session_length(
    const OlmOutboundGroupSession *session
) {
    return _olm_enc_output_length(raw_pickle_length(session));
}

__declspec(dllexport) size_t olm_pickle_outbound_group_session(
    OlmOutboundGroupSession *session,
...
__declspec(dllexport) size_t olm_unpickle_outbound_group_session(
    OlmOutboundGroupSession *session,
    void const * key, size_t key_length,
    void * pickled, size_t pickled_length
) {
...
__declspec(dllexport) size_t olm_init_outbound_group_session_random_length(
    const OlmOutboundGroupSession *session
) {
...
__declspec(dllexport) size_t olm_init_outbound_group_session(
    OlmOutboundGroupSession *session,
...
__declspec(dllexport) size_t olm_group_encrypt_message_length(
    OlmOutboundGroupSession *session,
    size_t plaintext_length
) {
...
__declspec(dllexport) size_t olm_group_encrypt(
    OlmOutboundGroupSession *session,
...
__declspec(dllexport) size_t olm_outbound_group_session_id_length(
    const OlmOutboundGroupSession *session
) {
    return _olm_encode_base64_length(GROUP_SESSION_ID_LENGTH);
}

__declspec(dllexport) size_t olm_outbound_group_session_id(
    OlmOutboundGroupSession *session,
...
__declspec(dllexport) uint32_t olm_outbound_group_session_message_index(
    OlmOutboundGroupSession *session
) {
...
__declspec(dllexport) size_t olm_outbound_group_session_key_length(
    const OlmOutboundGroupSession *session
) {
    return _olm_encode_base64_length(SESSION_KEY_RAW_LENGTH);
}

__declspec(dllexport) size_t olm_outbound_group_session_key(
    OlmOutboundGroupSession *session,
